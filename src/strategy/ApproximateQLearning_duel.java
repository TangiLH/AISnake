...
public class ApproximateQLearning_duel extends Strategy{
	...
	public ApproximateQLearning_duel(int n, double epsilon, double alpha, double gamma) {
	...
	public double[] extractFeatures(SnakeGame nextState, int idxSnake, AgentAction agentAction) {
		...
		double alive;
		f[4]=this.canEatSnake(nextState,snake_copy);
		//f[]=0;//snake.getSize();
		...
		if(!isLegalMove(snake,agentAction)) {
			...
			f[3]=0.5;
			 alive=this.snakeAlive(snake_copy,nextState,f[4]==1);
		}
...
private double canEatSnake(SnakeGame nextState, Snake snake_copy) {
	int maxSize=0;
	for(Snake s : nextState.getSnakes()) {
		maxSize=s.getSize()>maxSize ? s.getSize():maxSize;
	}
	if(snake_copy.getSize()>=maxSize) {
		return 1;
	}
	return 0;
}
...
public boolean isLegalMove(Snake snake, AgentAction action) {
	...
	private double snakeAlive(Snake snake_copy, SnakeGame nextState,boolean largestSnake) {
		...
		for(Position p:snake_copy.getPositions()) {
			...
			if(!largestSnake) {
				for(Snake s : nextState.getSnakes()) {
					if(s.getId()!=snake_copy.getId()) {
						for(Position p2 : s.getPositions()) {
							if(p.getX()==p2.getX()&&p.getY()==p2.getY()) {
								return 0;
							}
						}
